namespace Lab_6
{
    class SimplexMapper
    {
        public static void SolveSimplex(double[,] C)
        {
            int n = C.GetLength(0);
            int vars = n * n;
            int constraintsCount = 2 * n;
            ...
            double[,] table = new double[constraintsCount + 1, vars + 1];
            List<string> rLabels = [];
            List<string> cLabels = [.. Enumerable.Range(1, vars).Select(i => $"x{i}")];

            // Обмеження: сума по рядках <= 1
            for (int i = 0; i < n; i++)
            {
                List<string> eq = [];
                for (int j = 0; j < n; j++)
                ...
                table[i, vars] = 1.0;
                rLabels.Add($"y{i + 1}");
                Console.WriteLine($"{string.Join(" ", eq)} + 1 >= 0");
            }

            // Обмеження: сума по стовпцях >= 1
            for (int j = 0; j < n; j++)
            {
                List<string> eq = [];
                for (int i = 0; i < n; i++)
            ...
            while (true)
            {
                ...
                for (int j = 0; j < vars; j++)
                {
                    if (table[r, j] < -1e-7)
                    {
                        double ratio = Math.Abs(table[2 * n, j] / table[r, j]);
            ...
            {
                int s = -1;
                for (int j = 0; j < vars; j++) if (table[2 * n, j] < -1e-7) { s = j; break; }
                if (s == -1) break;
            ...
            Console.WriteLine($"Min (Z) = {Math.Abs(table[2 * n, vars]):F2}\n");
            PrintAssignmentPlanFromSimplex(table, rLabels, n, C, "Матриця призначень:");

        }

        /// <summary>
        /// Допоміжний метод для виведення матриці призначень прямо із симплекс-таблиці
        /// </summary>
        static void PrintAssignmentPlanFromSimplex(double[,] table, List<string> rLabels, int n, double[,] C, string title)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                {
                    double val = X[i * n + j];
                    Console.Write($"{val,5:F0}");
                    if (val > 0.5)
                    {
                        sum += C[i, j];
                        terms.Add(C[i, j].ToString());
                    }
                }
                Console.WriteLine();
            }
